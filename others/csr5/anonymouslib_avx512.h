#ifndef ANONYMOUSLIB_AVX512_H
#define ANONYMOUSLIB_AVX512_H

#include "detail/utils.h"
#include "detail/avx512/utils_avx512.h"

#include "detail/avx512/common_avx512.h"
#include "detail/avx512/format_avx512.h"
#include "detail/avx512/csr5_spmv_avx512.h"
...
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>::asCSR5()
{
    ...
    if (_format == ANONYMOUSLIB_FORMAT_CSR)
    {
        ...
        printf("omega = %i, sigma = %i. ", ANONYMOUSLIB_CSR5_OMEGA, _csr5_sigma);
        ...
        _csr5_partition_pointer = (unsigned int *) _mm_malloc((_p + 1) * sizeof(unsigned int), ANONYMOUSLIB_X86_CACHELINE);

        _csr5_partition_descriptor = (unsigned int *)_mm_malloc(_p * ANONYMOUSLIB_CSR5_OMEGA * _num_packet * sizeof(unsigned int), ANONYMOUSLIB_X86_CACHELINE);
        memset(_csr5_partition_descriptor, 0, _p * ANONYMOUSLIB_CSR5_OMEGA * _num_packet * sizeof(unsigned int));

        int num_thread = omp_get_max_threads();
        //printf("cali num_thread = %i\n", num_thread);
        _temp_calibrator = (VALUE_TYPE *)_mm_malloc(num_thread * ANONYMOUSLIB_X86_CACHELINE, ANONYMOUSLIB_X86_CACHELINE);
        memset(_temp_calibrator, 0, num_thread * ANONYMOUSLIB_X86_CACHELINE);

        _csr5_partition_descriptor_offset_pointer = (int *)_mm_malloc((_p + 1) * sizeof(int), ANONYMOUSLIB_X86_CACHELINE);
        memset(_csr5_partition_descriptor_offset_pointer, 0, (_p + 1) * sizeof(int));
        ...
        printf( " tile_ptr_time = %f\n", tile_ptr_time);

        _tail_partition_start = (_csr5_partition_pointer[_p-1] << 1) >> 1;
        //printf("_tail_partition_start = %i\n", _tail_partition_start);
        ...
        if (_num_offsets)
        {
            //printf("has empty rows, _num_offsets = %i\n", _num_offsets);
            malloc_timer.start();
            _csr5_partition_descriptor_offset = (int *)_mm_malloc(_num_offsets * sizeof(int), ANONYMOUSLIB_X86_CACHELINE);
            ...
            //    printf("_csr5_partition_descriptor_offset = %i\n", _csr5_partition_descriptor_offset[i]);
            if (err != ANONYMOUSLIB_SUCCESS)
                return ANONYMOUSLIB_CSR_TO_CSR5_FAILED;
            tile_desc_time += tile_desc_timer.stop();
        }

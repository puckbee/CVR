...
{
public:
  idx_sorter(vector<int>& idx) : _idx(idx) { }
  bool operator()(const int i, const int j) const {
    return _idx[i] < _idx[j];
  }
private:
  const vector<int>& _idx;
};

struct row_col_sorter
{
public:
  row_col_sorter(const int *row, const int *col) : _row(row), _col(col) { }
  bool operator()(const int i, const int j) const {
    if (_row[i] < _row[j]) return true;
    if (_row[i] > _row[j]) return false;
    if (_col[i] < _col[j]) return true;
    if (_col[i] > _col[j]) return false;
    return 0;
  }
private:
  const int *_row;
...
struct col_row_sorter
{
public:
  col_row_sorter(const int *row, const int *col) : _row(row), _col(col) { }
  bool operator()(const int i, const int j) const {
    if (_col[i] < _col[j]) return true;
    if (_col[i] > _col[j]) return false;
    if (_row[i] < _row[j]) return true;
...
/******************** Matrix Market internal definitions ********************
   ...
   MM_matrix_typecode: 4-character sequence object
		 		sparse/       data        storage 
	     	  		dense         type        scheme

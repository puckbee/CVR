...
double runKernel(anonymouslibHandle<int, unsigned int, VALUE_TYPE> A, VALUE_TYPE* y_bench, VALUE_TYPE alpha)
{
...
//    printf("CSR5 PRE Time %s  %f s. \n", filename, t2 - t1);
    printf("The Pre-processing(CSR->CSR5)  Time of CSR5  is %f seconds.   [file: %s] [threads: %d]\n", t2 - t1, filename, numThreads);
    ...
    if (NUM_RUN)
    {
       CSR5Spmv_time = runKernel(A, y_bench, alpha);
...
#ifdef MMAP
    VALUE_TYPE *x = (VALUE_TYPE *)mmap(0, n*sizeof(VALUE_TYPE), PROT_READ|PROT_WRITE,MAP_ANONYMOUS|MAP_PRIVATE|MAP_HUGETLB|262144,-1,0);
#else
